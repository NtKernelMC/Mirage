...
protected:
	...
	CAnsiRegistry(HKEY hKey, LPCSTR lpSubKey, bool mode);
	~CAnsiRegistry();
};
class CEasyRegistryAnsi : CAnsiRegistry
{
public:
	bool no_error;
	bool ErrorSuccess();
	void WriteString(LPCSTR lpName, LPCSTR lpString, bool multi_sz = false, ...);
	...
	bool IsError();
	CEasyRegistryAnsi(HKEY hKey, LPCSTR lpSubKey, bool mode);
	~CEasyRegistryAnsi();
};
