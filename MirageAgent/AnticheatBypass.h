...
#pragma pack(pop)
	...
	int reliability, char orderingChannel, int targetIp, __int16 targetPort, char broadcast, int receiptNumber)
	{
		...
		if (isPublicSerialPacket)
		{
			...
			LogInFile(LOG_NAME, xorstr_("[LOG] SET_PUBLIC candidate packet detected. packetId=%u bitLength=%d enabled=%d\n"),
				packetId, bitLength, should_set_public ? 1 : 0);
			...
			if (should_set_public)
			{
				...
				{
					std::lock_guard<std::mutex> lock(g_public_serial_log_mutex);
					if (g_last_logged_public_serial != public_serial)
					{
						g_last_logged_public_serial = public_serial;
						LogInFile(LOG_NAME, xorstr_("[LOG] SET_PUBLIC applied public serial: %s\n"), public_serial.c_str());
					}
				}
			}
	...
	void EvadeAnticheat()
	{
		...
		{
			if (EnsureMinHook(ProcessPacket_o, &ProcessPacket_h, reinterpret_cast<void**>(&ProcessPacket_o), "ProcessPacket"))
		...
		if (callSendPacket != nullptr)
		{
			...
			EnsureInlineJumpHook((DWORD)callSendPacket, (DWORD)&SendPacket, packet_prologue, sizeof(packet_prologue), "SendPacket");
		}
		...
		{
			EnsureMinHook(vpnbypass, &hkvpnbypass, reinterpret_cast<void**>(&vpnbypass), "ByPass Component #0");
		...
		if (target)
		{
			EnsureMinHook(target, &hkIsNetworkConnected, nullptr, "ByPass Component #1");
		...
		if (target)
		{
			EnsureMinHook(target, &hkIsNotViolationCode, nullptr, "ByPass Component #2");
		...
		if (target)
		{
			EnsureMinHook(target, &hkSendClientKick, nullptr, "ByPass Component #4");
		...
		if (callSendReport)
		{
			EnsureMinHook(callSendReport, &hkSendReport, nullptr, "ByPass Component #5");
		...
		if (target)
		{
			EnsureMinHook(target, &hkVfB00_Z00Scanner, nullptr, "ByPass Component #7");
		...
		if (target)
		{
			EnsureMinHook(target, &hkSetClientKickNew, nullptr, "ByPass Component #8");
		...
		if (target)
		{
			EnsureMinHook(target, &hkSendClientKick, nullptr, "ByPass Component #9");
...
namespace LegacyBypass
{
	...
	void EvadeAnticheat()
	{
		...
		if (callSendPacket == nullptr)
		{
			...
			{
				LogInFile(LOG_NAME, xorstr_("[LOG] Found address from signature to SendPacket!\n"));
				EnsureMinHook(callSendPacket, &SendPacket, reinterpret_cast<void**>(&callSendPacket), "Legacy SendPacket");
			}
			else
			{
